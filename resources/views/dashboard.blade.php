        ...
        <aside class="w-full md:w-64 border-r-2 border-black bg-[#f4f3ef] flex flex-col md:min-h-screen relative">
            ...
            <nav class="flex-grow flex flex-col p-4 space-y-2">
                <a href="{{ route('dashboard') }}"
                    class="font-mono text-sm font-bold uppercase bg-black text-white px-3 py-2 border-2 border-black">Overview</a>
                <a href="{{ route('my-courses') }}"
                    class="font-mono text-sm font-bold uppercase hover:bg-gray-200 px-3 py-2 border-2 border-transparent transition-colors">My Courses</a>
        ...
        <div class="flex-grow p-4 md:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="brutal-border p-6 bg-white brutal-shadow-sm flex flex-col relative overflow-hidden group">
                    ...
                    <div
                        class="absolute -right-4 -bottom-4 text-8xl text-gray-100 group-hover:text-yellow-100 transition-colors font-bold z-0 pointer-events-none">
                        {{ count(auth()->user()->enrolled_courses ?? []) }}</div>
                </div>
                <div class="brutal-border p-6 bg-white brutal-shadow-sm flex flex-col relative overflow-hidden group">
                    <div class="font-mono text-xs uppercase mb-2">Pending Assignments</div>
                    <div class="text-5xl font-bold text-red-600">0</div>
                    <div
                        class="absolute -right-4 -bottom-4 text-8xl text-gray-100 group-hover:text-red-50 transition-colors font-bold z-0 pointer-events-none">
                        0</div>
                </div>
            </div>
            ...
            @if($lastCourse)
                ...
                <div class="brutal-border bg-white flex flex-col md:flex-row mb-8 brutal-shadow hover-lift">
                    <div class="bg-black w-full md:w-32 flex items-center justify-center p-4">
                        <span
                            class="text-white font-mono text-4xl font-bold">{{ strtoupper(substr($lastCourse->slug, 0, 2)) }}</span>
                    </div>
                    ...
                    <div class="p-6 flex items-center justify-center border-t-2 md:border-t-0 md:border-l-2 border-black">
                        <a href="{{ route('course', $lastCourse->slug) }}"
                            class="brutal-border bg-yellow-300 px-4 py-2 font-mono text-sm font-bold uppercase hover:bg-yellow-400 transition-colors">Resume</a>
                    </div>
                </div>
            @else
                ...
                <div class="brutal-border bg-white p-6 md:p-8 mb-8 brutal-shadow text-center">
                    ...
                    <a href="{{ route('catalog') }}"
                        class="brutal-border bg-blue-600 text-white px-6 py-3 font-mono font-bold uppercase hover-lift inline-block">Browse
                        Catalog</a>
                </div>
            @endif
        </div>
